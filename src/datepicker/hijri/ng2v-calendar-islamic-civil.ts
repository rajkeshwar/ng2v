import {Ng2vCalendarHijri} from './ng2v-calendar-hijri';
import {Ng2vDate} from '../ng2v-date';
import {Ng2vPeriod} from '../ng2v-calendar';
...
@Injectable()
export class Ng2vCalendarIslamicCivil extends Ng2vCalendarHijri {
  /**
   * Returns the equivalent islamic(civil) date value for a give input Gregorian date.
   * `gdate` is a JS Date to be converted to Hijri.
   */
  fromGregorian(gdate: Date): Ng2vDate {
    ...
    return new Ng2vDate(hYear, hMonth + 1, hDay);
  }
   ...
   */
  toGregorian(hijriDate: Ng2vDate): Date {
  ...
  getWeekNumber(week: Ng2vDate[], firstDayOfWeek: number) {
    ...
    const MuhDate = this.toGregorian(new Ng2vDate(date.year, 1, 1));  // Compare with Muharram 1
    return Math.floor(Math.round((time - MuhDate.getTime()) / 86400000) / 7) + 1;
  }

  getToday(): Ng2vDate { return this.fromGregorian(new Date()); }
}
