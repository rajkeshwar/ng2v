...
const NGB_DATEPICKER_VALUE_ACCESSOR = {
  ...
  useExisting: forwardRef(() => Ng2vDatepicker),
  multi: true
};
 ...
 */
export interface Ng2vDatepickerNavigateEvent {
  /**
...
@Component({
  ...
  template: `
    <ng-template #dt let-date="date" let-currentMonth="currentMonth" let-selected="selected" let-disabled="disabled">
       <div ng2vDatepickerDayView [date]="date" [currentMonth]="currentMonth" [selected]="selected" [disabled]="disabled"></div>
    </ng-template>
    
    <div class="ng2v-dp-header bg-faded pt-1 rounded-top" [style.height.rem]="getHeaderHeight()" 
      [style.marginBottom.rem]="-getHeaderMargin()">
      <ng2v-datepicker-navigation *ngIf="navigation !== 'none'"
      ...
      </ng2v-datepicker-navigation>
    </div>

    <div class="ng2v-dp-months d-flex px-1 pb-1">
      <ng-template ngFor let-month [ngForOf]="months" let-i="index">
        <div class="ng2v-dp-month d-block ml-3">            
          <div *ngIf="navigation !== 'select' || displayMonths > 1" class="ng2v-dp-month-name text-center">
  ...
  `,
  providers: [NGB_DATEPICKER_VALUE_ACCESSOR, Ng2vDatepickerService]
})
export class Ng2vDatepicker implements OnChanges,
  ...
  constructor(
      private _service: Ng2vDatepickerService, private _calendar: Ng2vCalendar, public i18n: Ng2vDatepickerI18n,
      config: Ng2vDatepickerConfig) {
  ...
  onNavigateDateSelect(date: Ng2vDate) {
    this._setViewWithinLimits(date);
    this._updateData();
  }
  ...
  private _setDates() {
    this._maxDate = Ng2vDate.from(this.maxDate);
    this._minDate = Ng2vDate.from(this.minDate);
  ...
  private _setViewWithinLimits(date: Ng2vDate) {
    if (this._minDate && date.before(this._minDate)) {
      this._date = new Ng2vDate(this._minDate.year, this._minDate.month, 1);
    } else if (this._maxDate && date.after(this._maxDate)) {
      this._date = new Ng2vDate(this._maxDate.year, this._maxDate.month, 1);
    } else {
      this._date = new Ng2vDate(date.year, date.month, 1);
    }
  }
