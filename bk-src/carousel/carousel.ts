 ...
 */
@Directive({selector: 'ng-template[ng2vSlide]'})
export class Ng2vSlide {
  /**
   * Unique slide identifier. Must be unique for the entire document for proper accessibility support.
   * Will be auto-generated if not provided.
   */
  @Input() id = `ng2v-slide-${nextId++}`;
...
export class Ng2vCarousel implements AfterContentChecked,
    OnDestroy, OnInit {
  @ContentChildren(Ng2vSlide) slides: QueryList<Ng2vSlide>;
  ...
  private _getSlideById(slideId: string): Ng2vSlide {
    let slideWithId: Ng2vSlide[] = this.slides.filter(slide => slide.id === slideId);
...
export const NGB_CAROUSEL_DIRECTIVES = [Ng2vCarousel, Ng2vSlide];
