...
describe('ng2v-typeahead-window', () => {

  beforeEach(() => {
    TestBed.overrideModule(Ng2vTypeaheadModule, {set: {exports: [Ng2vTypeaheadWindow]}});
  ...
  describe('display', () => {
    ...
    it('should display results with the first row active', () => {
      const fixture =
          createTestComponent('<ng2v-typeahead-window [results]="results" [term]="term"></ng2v-typeahead-window>');
    ...
    it('should use a formatting function to display results', () => {
      const fixture = createTestComponent(
          '<ng2v-typeahead-window [results]="results" [term]="term" [formatter]="formatterFn"></ng2v-typeahead-window>');
    ...
    it('should use a custom template if provided', () => {
      const fixture = createTestComponent(`
           ...
           <ng2v-typeahead-window [results]="results" [term]="term" [resultTemplate]="rt"></ng2v-typeahead-window>`);
  ...
  describe('active row', () => {
    ...
    it('should change active row on prev / next method call', () => {
      const html = `
           ...
           <ng2v-typeahead-window [results]="results" [term]="term" #w="ng2vTypeaheadWindow"></ng2v-typeahead-window>`;
    ...
    it('should wrap active row on prev / next method call for [focusFirst]="false"', () => {
      const html = `
           <button (click)="w.next()">+</button>
           <button (click)="w.prev()">-</button>
           <ng2v-typeahead-window [results]="results" [term]="term" #w="ng2vTypeaheadWindow" [focusFirst]="false"></ng2v-typeahead-window>`;
      const fixture = createTestComponent(html);
      const buttons = fixture.nativeElement.querySelectorAll('button');
  ...
  describe('result selection', () => {
    it('should select a given row on click', () => {
      const fixture = createTestComponent(
          '<ng2v-typeahead-window [results]="results" [term]="term" (select)="selected = $event"></ng2v-typeahead-window>');
      const links = getWindowLinks(fixture.debugElement);
    ...
    it('should have buttons of type button', () => {
      const html = `
           <ng2v-typeahead-window [results]="results" [term]="term"></ng2v-typeahead-window>`;
      const fixture = createTestComponent(html);
      const buttons = fixture.nativeElement.querySelectorAll('button');
  ...
  describe('accessibility', () => {
    ...
    it('should add correct ARIA attributes', () => {
      const fixture = createTestComponent(
          '<ng2v-typeahead-window id="test-typeahead" [results]="results" [term]="term"></ng2v-typeahead-window>');
      const compiled = fixture.nativeElement.querySelector('ng2v-typeahead-window.dropdown-menu');
...
class TestComponent {
  ...
  @ViewChild(Ng2vTypeaheadWindow) popup: Ng2vTypeaheadWindow;
