...
import {Ng2vAccordionModule} from './accordion.module';
import {Ng2vAccordionConfig} from './accordion-config';
import {Ng2vAccordion} from './accordion';
...
describe('ng2v-accordion', () => {
  let html = `
    <ng2v-accordion #acc="ng2vAccordion" [closeOthers]="closeOthers" [activeIds]="activeIds"
      (panelChange)="changeCallback($event)" [type]="classType">
      <ng2v-panel *ngFor="let panel of panels" [id]="panel.id" [disabled]="panel.disabled" [type]="panel.type">
        <ng-template ng2vPanelTitle>{{panel.title}}</ng-template>
        <ng-template ng2vPanelContent>{{panel.content}}</ng-template>
      </ng2v-panel>
    </ng2v-accordion>
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({declarations: [TestComponent], imports: [Ng2vAccordionModule.forRoot()]});
    TestBed.overrideComponent(TestComponent, {set: {template: html}});
  });

  it('should initialize inputs with default values', () => {
    const defaultConfig = new Ng2vAccordionConfig();
    const accordionCmp = new Ng2vAccordion(defaultConfig);
  ...
  it('can use a title without template', () => {
    const testHtml = `
    <ng2v-accordion>
     <ng2v-panel [title]="panels[0].title">
       <ng-template ng2vPanelContent>{{panels[0].content}}</ng-template>
     </ng2v-panel>
    </ng2v-accordion>
    `;
  ...
  it('can mix title and template', () => {
    ...
    <ng2v-accordion>
     ...
     <ng2v-panel>
      <ng-template ng2vPanelTitle>{{panels[1].title}}</ng-template>
      <ng-template ng2vPanelContent>{{panels[1].content}}</ng-template>
     </ng2v-panel>
    </ng2v-accordion>
    `;
    const fixture = createTestComponent(testHtml);
  ...
  it('should have specified type of accordion ', () => {
    const testHtml = `
    <ng2v-accordion #acc="ng2vAccordion" [closeOthers]="closeOthers" [type]="classType">
     <ng2v-panel *ngFor="let panel of panels" [id]="panel.id" [disabled]="panel.disabled">
       <ng-template ng2vPanelTitle>{{panel.title}}</ng-template>
       <ng-template ng2vPanelContent>{{panel.content}}</ng-template>
     </ng2v-panel>
    </ng2v-accordion>
  ...
  describe('Custom config', () => {
    let config: Ng2vAccordionConfig;

    beforeEach(() => { TestBed.configureTestingModule({imports: [Ng2vAccordionModule.forRoot()]}); });

    beforeEach(inject([Ng2vAccordionConfig], (c: Ng2vAccordionConfig) => {
      config = c;
    ...
    it('should initialize inputs with provided config', () => {
      const fixture = TestBed.createComponent(Ng2vAccordion);
      fixture.detectChanges();
  ...
  describe('Custom config as provider', () => {
    let config = new Ng2vAccordionConfig();
    config.closeOthers = true;
    config.type = 'success';

    beforeEach(() => {
      TestBed.configureTestingModule(
          {imports: [Ng2vAccordionModule.forRoot()], providers: [{provide: Ng2vAccordionConfig, useValue: config}]});
    });
